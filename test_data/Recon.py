...
class Recon(object):
    ...
    @staticmethod
    def irdm(sinogram_np, angles, numIter=1):
        # print("--start irdm---")
        recon_wb = iradon(sinogram_np, theta=angles, circle=True) / 255.
        ...
        ori_tensor = ToTensor()(recon_wb.astype(np.float32)).cuda()
    ...
    def irdm_80(sinogram_np, angles, numIter=1):
        # print("--start irdm---")
        recon_wb = iradon(sinogram_np, theta=angles, circle=True) / 255.
    ...
    @classmethod
    def fusion_3dGAN(cls, sinogram_np, angles, numIter=1):
        # print("--start irdm---")
        wbp_input = Normalize(cls.wbp(sinogram_np, angles).astype('float16')[:, :, np.newaxis])
        irdm_input = Normalize(cls.irdm(sinogram_np, angles).astype('float16')[:, :, np.newaxis])
        irdm_80_input = Normalize(cls.irdm_80(sinogram_np, angles).astype('float16')[:, :, np.newaxis])
